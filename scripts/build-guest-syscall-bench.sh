...
SRC_DIR="$ROOT_DIR/rootfs/guest-src/bench"
OUT_DIR="$ROOT_DIR/build/guest-syscall-bench"
...
"$ZIG_BIN" cc -target aarch64-linux-gnu -O2 -s \
  "$SRC_DIR/alr_syscall_bench.c" \
  -o "$OUT_DIR/alr-syscall-bench"

file "$OUT_DIR/alr-syscall-bench"
