...
def test_debug_apk_version_code_is_bumped_for_device_update():
    ...
    assert 'versionName = "0.2.0-proot-smoke"' in text


def test_main_activity_starts_with_visible_build_stamp_before_summary():
    text = MAIN.read_text()
    assert "build: 0.2.0-proot-smoke" in text
    assert text.index("build: 0.2.0-proot-smoke") < text.index("execution summary")
