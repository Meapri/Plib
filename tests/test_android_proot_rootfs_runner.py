...
def test_main_activity_reports_apt_base_bundle_smoke_results():
    ...
    assert "ALR APT-CACHE STATS PRELOAD EXECUTION:" in text
    assert "ALR APT-CACHE PKGNAMES PRELOAD EXECUTION:" in text
    ...
    assert "runAlrRuntimeTrampolineAptCacheStatsPreload" in runner
    assert "runAlrRuntimeTrampolineAptCachePkgNamesPreload" in runner
    ...
    assert "alr apt-cache stats preload path rewrite=" in text
    assert "alr apt-cache stats preload stdout=" in text
    assert "alr apt-cache pkgnames preload path rewrite=" in text
    assert "alr apt-cache pkgnames preload stdout=" in text
...
def test_main_activity_reports_local_deb_install_smoke_results():
    ...
    assert "ALR INSTALLED PACKAGE PRELOAD EXECUTION:" in text
    ...
    assert "runAlrRuntimeTrampolineInstalledPackageSmokePreload" in runner
    assert '"/bin/dash"' in runner
    ...
    assert "alr installed package preload handoff=" in text
    assert "alr installed package preload execve loader rewrites=" in text
    assert "alr installed package preload last exec requested=" in text
    assert "alr installed package preload last guest=" in text
    assert "alr installed package preload stdout=" in text
